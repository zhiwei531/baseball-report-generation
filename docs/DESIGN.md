...
- 当前可复用的最终报告由 `python scripts/report_cli.py final --config configs/final_report.json` 生成；client 也可分别调用 `pitching` 与 `batting` execution。路径和主体配置集中在 final config 及其引用的 batting config，核心 batting 编排在 `scripts/run_batting_report_pipeline.py`。final 会先从 Vicon C3D 生成 pitching HTML/assets，再生成 batting C3D 中间 CSV、3D reconstruction 资产、batting metrics、经人工确认事件帧的 MediaPipe 2D alignment、2D-vs-Vicon-3D QA、2D metric annotations、HTML schema、researcher charts 和 XLSX，并嵌入本次 pitching 资产。其它 builder 仅是实现细节，不是报告入口。
