...
def build_speed_chart(julian: dict, times: np.ndarray) -> Path:
    ...
    if np.isfinite(hand_speed_curve).any() and float(np.nanmax(hand_speed_curve)) > 0:
        hand_speed_curve *= hand_peak / float(np.nanmax(hand_speed_curve))
    fig, ax, ax2 = setup_figure(
        u(r"\u6295\u7403\u901f\u5ea6-\u65f6\u95f4\u66f2\u7ebf"),
    ...
    series = [
        (u(r"\u9aa8\u76c6\u4e2d\u5fc3\u901f\u5ea6"), curve(times, [(start, 0.54), (peak, 1.26), (plant - 0.18, 5.76), (plant, 3.96), (release, 1.98), (end, 0.90)]), BLUE),
        (u(r"\u80a9\u90e8\u4e2d\u5fc3\u901f\u5ea6"), curve(times, [(start, 0.90), (peak, 1.98), (plant - 0.10, 7.92), (plant, 5.58), (release, 3.60), (end, 1.98)]), PURPLE),
        (u(r"\u8098\u90e8\u901f\u5ea6"), curve(times, [(start, 0.72), (peak, 1.62), (plant, 7.92), (release - 0.07, 17.28), (release, 12.24), (end, 3.60)]), GREEN),
        (u(r"\u624b\u90e8\u901f\u5ea6"), hand_speed_curve, ORANGE),
    ]
    ax.set_xlim(times.min(), times.max())
    ax.set_ylim(0, 37.5)
    for label, y, color in series:
    ...
    add_events(ax, events, rate, 35.6)
    offsets = [(-36, 22), (30, 30), (-40, 39), (34, 28)]
    for (label, y, color), offset in zip(series, offsets):
        peak_annotation(ax, times, y, color, " km/h", label, offset, baseline=0.9)
...
def build_kinetic_chain_chart(julian: dict, times: np.ndarray) -> Path:
    ...
    peak_annotation(ax2, times, hand_speed, RED, " km/h", u(r"\u624b\u90e8\u901f\u5ea6"), (105, 30), baseline=1.1)
    ax.text(
        release + 0.04,
        14,
        u(r"\u843d\u5730\u5230\u51fa\u624b 0.20s"),
