...
from baseball_report.core.enums import MotionType
from baseball_report.visualization.manifest import (
    discover_report_assets,
    infer_asset_association,
)
...
class VisualizationManifestTests(unittest.TestCase):
    def test_discovers_portable_existing_assets_without_guessing_metric_links(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (root / "assets" / "_tmp_ready" / "temp.gif").parent.mkdir(parents=True)
            (root / "assets" / "_tmp_ready" / "temp.gif").write_bytes(b"temp")
    ...
    def test_canonical_asset_names_have_explicit_motion_metric_and_event_links(self) -> None:
        self.assertEqual(
            infer_asset_association("assets/frontend_metric_illustrations/ready_balance.png"),
            (
                MotionType.BATTING,
                ("ready_com_height_ratio", "ready_to_contact_head_displacement_mm"),
                ("Ready Position",),
            ),
        )
        self.assertEqual(
            infer_asset_association("assets/vicon_reconstruction_events/player_contact.gif"),
            (MotionType.BATTING, (), ("Contact Position",)),
        )
        self.assertEqual(
            infer_asset_association("pitch_assets/frontend_metric_illustrations_pitch/hand_speed_mps.png"),
            (MotionType.PITCHING, ("hand_speed_kmh",), ()),
        )
        motion, metrics, events = infer_asset_association(
            "pitch_assets/vicon_reconstruction_events/player_release.gif"
        )
        self.assertEqual((motion, metrics, events), (MotionType.PITCHING, (), ("release",)))
